...
function Nav({ scrolledToNav }: NavProps) {
  return (
    <div id='navBar' className={`w-full mt-12 border-opacity-0 border-b-2 border-sky-500 lg:mt-24 ${scrolledToNav ? 'fixedToTop' : ''}`}>
      
      <div className='container mx-auto flex'>

        <div id='navLogo' className={`m-auto opacity-0 pl-6 ${scrolledToNav ? 'w-4/12' : 'w-0'}`}>
          <LogoSVG title='Ross Logan Web' />
        </div>

        <div className={`pr-6 ${scrolledToNav ? 'w-8/12 text-right' : 'w-full text-center'}`}>
